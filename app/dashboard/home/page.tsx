        ...
        {events.map((item, index) => (
          ...
          >
            ...
            <div className='flex gap-2'>
              ...
              <p className='px-3 py-1 bg-zinc-600 rounded-full'>{item.feild}</p>
            </div>
            <div className='text-green-300'>{item.time}</div>
            <div className='text-lg font-bold'>{item.title}</div>
            <div className='text-yellow-400 pt-6'>{item.organizer}</div>
            <Link href={'/dashboard/event'} className='animate-pulse text-zinc-200 text-4xl font-bold scale-95 hover:scale-100'>⇨</Link>
          </div>
        ))}
      </div>
    )
